...
class databaseConnectorMatch(object):
    def __init__(self):
        try:
    ...
    def updateBatting(self, battingInning, teamName, matchID):
        for player in battingInning:
            self.cur.execute(f"UPDATE tblBatting SET Runs = Runs +{player.runs}, Balls = Balls + {player.balls}, 4s = 4s + {player.fours}, 6s = 6s + {player.sixes} WHERE BatsmanName = '{player.name}'") 
            self.db.commit()
            if player.runs>99:
                self.cur.execute(f"UPDATE tblBatting SET 100s = 100s + 1 WHERE BatsmanName = '{player.name}'") 
                self.db.commit()
            elif player.runs>49:
                self.cur.execute(f"UPDATE tblBatting SET 50s = 50s + 1 WHERE BatsmanName = '{player.name}'") 
                self.db.commit()
            if player.status == "Out":
                self.cur.execute(f"UPDATE tblBatting SET Outs = Outs + 1 WHERE BatsmanName = '{player.name}'") 
                self.db.commit()
            if player.runs>0:
                self.cur.execute(f"INSERT INTO tblHighestScore (MatchID,BatsmanID,Highest) VALUES ({matchID},'{player.name}',{player.runs})") 
                self.db.commit()
            #self.db.commit()

    def updateBowling(self, bowlingInning, teamName, matchID):
        for player in bowlingInning:
            self.cur.execute(f"UPDATE tblBowling SET Balls = Balls + {player.overs}*6 + {player.balls}, Runs = Runs + {player.runs}, Maidens = Maidens + {player.maidens}, Wickets = Wickets + {player.wickets} WHERE BowlerName = '{player.name}'") 
            self.db.commit()
            if player.wickets>4:
                self.cur.execute(f"UPDATE tblBowling SET 5W = 5W + 1 WHERE BowlerName = '{player.name}'") 
                self.db.commit()
            if player.wickets>0:
                self.cur.execute(f"INSERT INTO tblBestBowling (MatchID,BowlerID,Wickets,Runs) VALUES ({matchID},'{player.name}',{player.wickets},{player.runs})") 
                self.db.commit()


    def updateAfterMatch(self, matchID, battingInning1, bowlingInning1, battingInning2, bowlingInning2, team1, team2, winner, inn1Score, inn1Overs, inn2Score, inn2Overs, status):
        #print(battingInning1)
        #print(bowlingInning1)
        self.updateBatting(battingInning1, team1, matchID)
        self.updateBowling(bowlingInning1, team2, matchID)
        self.updateBatting(battingInning2, team2, matchID)
        self.updateBowling(bowlingInning2, team1, matchID)
        self.cur.execute(f"UPDATE tblMatches SET Status = 'Completed' WHERE MatchID = {matchID}")
        self.db.commit()
        self.cur.execute(f"INSERT INTO tblResults VALUES ({matchID},'{team1}','{team2}','{inn1Score}','{inn1Overs}','{inn2Score}','{inn2Overs}','{winner}','{status}')")
        self.db.commit()
...
class Ui_matchPage(QtWidgets.QWidget):

    def showDialog(self):
        self.msgBox = QMessageBox()
        self.msgBox.setIcon(QMessageBox.Warning)
        self.msgBox.setText(f"{self.winner} has won the match. \n Kindly close the window.")
        self.msgBox.setWindowTitle("Match Completed!!")
    ...
    def onClickButton(self, val):
        ...
        if self.isWicket.isChecked():
            ...
            self.striker.status="Out"
            self.striker=Batsman(self.availableBatsman.pop(0))
            #check for wicket type
            if self.inning1:
                self.battingInnings1.append(self.striker)
            else:
                self.battingInnings2.append(self.striker)
            #update DataBase
        ...
        if endOver:
            ...
            if (self.inning1 and self.team1.overs!=self.maxOvers) or (self.inning1==False and self.team2.overs!=self.maxOvers):
                self.newBowler = self.getBowler(self.availableBowlers[:])
                if self.newBowler in self.bowlersBowled:
                    ...
                    if self.inning1:
                        ...
                        self.bowlingInning2.append(self.currBowler)
    ...
    def startInning(self,avlblBatsmen,avlblBowler):
        ##Added by Tafzeel
        #########################################################COMPLETED#################################################
        #items = ("Red","Blue","Green")
        striker, okPressed = QtWidgets.QInputDialog.getItem(self, "Inning Start","Striker:", avlblBatsmen,0,False)
        while not okPressed:
            striker, okPressed = QtWidgets.QInputDialog.getItem(self, "Inning Start","Striker:", avlblBatsmen,0,False)
        okPressed=False
        avlblBatsmen.remove(striker)
        nonStriker, okPressed = QtWidgets.QInputDialog.getItem(self, "Inning Start","Non Striker:", avlblBatsmen,0,False)
        while not okPressed:
            nonStriker, okPressed = QtWidgets.QInputDialog.getItem(self, "Inning Start","Non Striker:", avlblBatsmen,0,False)
        okPressed=False
        bowler, okPressed = QtWidgets.QInputDialog.getItem(self, "Inning Start","Bowler:", avlblBowler,0,False)
        while not okPressed:
            bowler, okPressed = QtWidgets.QInputDialog.getItem(self, "Inning Start","Bowler:", avlblBowler,0,False)
        return striker,nonStriker,bowler

    def getBowler(self,items):
        ##Added by Tafzeel
        #########################################################COMPLETED#################################################
        #items = ("Red","Blue","Green")
        item, okPressed = QtWidgets.QInputDialog.getItem(self, "Select Bowler","New Bowler:", items,0,False)
        while not okPressed:
            item, okPressed = QtWidgets.QInputDialog.getItem(self, "Select Bowler","New Bowler:", items,0,False)
        return item
    ...
    def changeInnings(self):
        ...
        self.prevBowler=None
        strkrName, nonStrkr, currBwlr = self.startInning(self.availableBatsman[:], self.availableBowlers[:])
        self.currBowler=Bowler(currBwlr)        
        self.striker=Batsman(strkrName)
        self.nonStriker=Batsman(nonStrkr)
        self.bowlersBowled.add(currBwlr)
        self.availableBowlers.remove(self.currBowler.name)
    ...
    def newInnings(self):
        ...
        #self.nonStriker.status='Not Out'
        self.availableBowlers.remove(self.currBowler.name)
        self.availableBatsman.remove(self.striker.name)
        self.availableBatsman.remove(self.nonStriker.name)
    ...
    def endMatch(self):
        ...
        self.matchStatus.setText(f"{self.winner} won the match!")
        inn1Score=f"{self.team1.score}/{self.team1.wickets}"
        inn2Score=f"{self.team2.score}/{self.team2.wickets}"
        inn1Overs=f"{self.team1.overs}.{self.team1.balls}"
        inn2Overs=f"{self.team2.overs}.{self.team2.balls}"
        status=f'{self.winner} WON'
        self.database.updateAfterMatch(self.matchID, self.battingInnings1, self.bowlingInning1, self.battingInnings2, self.bowlingInning2, self.team1.name, self.team2.name, self.winner, inn1Score, inn1Overs, inn2Score, inn2Overs, status)
    ...
    def setupUi(self, matchPage, toss, team1, team2):
        ...
        font = QtGui.QFont()
        font.setPointSize(18)
        self.swapBatsmanButton.setFont(font)
        ...
        font = QtGui.QFont()
        font.setPointSize(18)
        self.applyDLButton.setFont(font)
        ...
        font = QtGui.QFont()
        font.setPointSize(18)
        self.endMatchButton.setFont(font)
        ...
        font = QtGui.QFont()
        font.setPointSize(25)
        self.buttonEndInnings.setFont(font)
        ...
        self.database=databaseConnectorMatch()
...
if __name__ == "__main__":
    ...
    ui = Ui_matchPage()
    team1 = ['CSK','MS Dhoni','MS Dhoni','Shane Watson','Faf Du Plessis','Suresh Raina', 'Ambati Rayudu','Kedar Jadhav','Dwayne Bravo','Ravindra Jadeja','Deepak Chahar', 'Shardul Thakur','Imran Tahir']
    ...
    toss = ['CSK','Feild First',2,18]
    ui.setupUi(matchPage,toss[:],team1[:],team2[:])
    #ui.setupUi(matchPage)
    matchPage.show()
    sys.exit(app.exec_())
